...
// 사용자가 backend 의 보안 자동 도구가 실시간으로 동작하는 것을 한 눈에 본다.
function EventStreamPanel() {
  ...
  const [kindFilter, setKindFilter] = useState('all'); // all | audit | suspicious
  ...
  const fetchOnce = async () => {
    ...
    try {
      const [a, s] = await Promise.all([
        ...
        api.get('/api/suspicious-events?limit=100'),
      ]);
  ...
  useEffect(() => {
    ...
    fetchOnce();
    const id = setInterval(fetchOnce, 5_000);  // 5초 cadence — 거의 실시간
  ...
  return (
    <div style={{ marginTop: 28 }}>
      <h3 className="admin-section-title">실시간 이벤트 로그 (터미널)</h3>
      <p style={{ fontSize: 12, color: '#8c867d', marginTop: 4 }}>
        backend 의 audit_logs (인증/관리자 행동) + suspicious_events (자동 보안 탐지) 를
        5초 주기로 가져와 시간 순서로 보여줍니다. backend 보안 자동 도구가 정상 동작 중인지
        실시간 확인용. <strong>brute_force_login</strong> 의심 이벤트는 동일 IP 가 짧은 시간 내
        15회 인증 실패 시 자동 기록됩니다.
      </p>
      ...
      <div ref={scrollRef} style={{
        ...
        fontSize: 12, lineHeight: 1.55, padding: 14, height: 360, overflow: 'auto',
