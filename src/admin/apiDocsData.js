    ...
    tables: [
      {
        ...
        columns: [
          ...
          { col: 'totp_app_label', type: 'varchar(40)', note: '인증 앱 라벨 (Google Authenticator / Authy / 1Password 등) — 분실 시 운영자 식별' },
  ...
  },
  {
    ...
    tables: [
      ...
      },
      {
        ...
        purpose: '의심 이벤트 — 운영자가 검토 후 resolve 처리. login throttle lock 도달 시 자동 brute_force_login 기록.',
        columns: [
          { col: 'id', type: 'int PK' },
          { col: 'detected_at', type: 'datetime INDEX' },
          { col: 'reason', type: 'varchar(40)', note: 'brute_force_login / token_leak / scrape_pattern / rate_limit_exceeded / abnormal_payload / uploaded_malware_signature' },
          { col: 'severity', type: 'varchar(12)', note: 'low / medium / high / critical' },
          { col: 'ip', type: 'varchar(45)' },
          { col: 'user_agent_hash', type: 'varchar(64)', note: 'sha256(UA) — 원본 UA 저장 안 함 (PII 최소화)' },
          { col: 'evidence', type: 'bool', note: 'true=법적 절차용, sweep 면제 (365일 보존)' },
          { col: 'sealed_at / sealed_by', type: 'datetime / varchar', note: 'evidence 봉인 시 변경 불가' },
        ],
        usedBy: ['/admin/security 의심 IP 패널', '/admin/monitoring'],
        pii: 'IP + UA hash. evidence=true 365일, 그 외 90일 보존 (auto sweep cron).',
      },
    ],
  },
  {
    key: 'commerce',
    label: '커머스 / 재고',
    desc: 'SKU 표준 + LOT + 유통기한. FIFO 차감 + D-3 임박 알림 + 만료 자동 격리.',
    tables: [
      {
        name: 'products',
        purpose: '발주 카탈로그 상품. SKU 표준 형식 DAEMU-CAT-NNNN-LL.',
        columns: [
          { col: 'id', type: 'int PK' },
          { col: 'sku', type: 'varchar(40) UNIQUE', note: 'DAEMU-{BAK|CAF|EQP|PCK|MSC}-{NNNN}-{LL}' },
          { col: 'name', type: 'varchar(120)' },
          { col: 'category', type: 'varchar(8)', note: 'BAK/CAF/EQP/PCK/MSC' },
          { col: 'unit', type: 'varchar(20)', note: '개 / kg / box 등' },
          { col: 'price', type: 'int', note: '단가(원)' },
          { col: 'stock_count', type: 'int', note: '< 10 시 알림' },
          { col: 'created_at / updated_at', type: 'datetime' },
        ],
        usedBy: ['/admin/inventory (SKU/상품 탭)', '발주 시 재고 초과 차단'],
        pii: '없음.',
      },
      {
        name: 'stock_lots',
        purpose: 'LOT 단위 입고 — 식품 B2B 의 FIFO + 유통기한 추적.',
        columns: [
          { col: 'id', type: 'int PK' },
          { col: 'sku', type: 'varchar(40) INDEX' },
          { col: 'lot_number', type: 'varchar(40)', note: '(sku, lot_number) UNIQUE' },
          { col: 'quantity', type: 'int' },
          { col: 'produced_at / expires_at / received_at', type: 'datetime?' },
          { col: 'supplier', type: 'varchar(120)' },
          { col: 'note', type: 'text' },
          { col: 'quarantined', type: 'bool', note: '만료 시 cron 이 자동 true (발주 불가)' },
          { col: 'created_by', type: 'int FK→admin_users.id' },
        ],
        usedBy: ['/admin/inventory (LOT 탭)', '발주 처리 시 expires_at 빠른 LOT 부터 차감'],
        pii: '없음 (supplier 는 회사명).',
      },
      {
        name: 'announcements',
        purpose: '공지/프로모션 — 공개 사이트 + 파트너 포털 노출.',
        columns: [
          { col: 'id', type: 'int PK' },
          { col: 'title / body', type: 'varchar(190) / text' },
          { col: 'kind', type: 'varchar(12)', note: 'notice / promo / urgent (urgent = 빨간 강조)' },
          { col: 'target', type: 'varchar(20)', note: 'all / public / partner_portal' },
          { col: 'active', type: 'bool' },
          { col: 'scheduled_start / scheduled_end', type: 'datetime?' },
      ...
      {
        name: 'partner_brands',
        purpose: 'Home 페이지의 "함께하는 파트너사" 로고 카드. (Partner 모델은 로그인 계정, 이건 단순 디스플레이.)',
        columns: [
          { col: 'id', type: 'int PK' },
          { col: 'name', type: 'varchar(190)' },
